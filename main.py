...
from upload_to_drive import TransferData
...
def get_all_items():
    ...
    return render_template("index.html", form=all_items, current_user=current_user)
...
def add_to_cart():
    ...
    if request.method == "POST":
        ...
        # print(data["item_id"])
        session['cart'].append({'id': data["item_id"], 'quantity': 1})
...
def download_request(req_id):
    ...
    return render_template("view_requests_popup.html", requested_items=item_details, current_user=current_user)
...
def get_all():
    ...
    template = env.get_template("table_toggle_format.html")  # Replace with the name of your template
    rendered_template = template.render(form=request_data, current_user=current_user)  # Replace with your actual form data
    return rendered_template  # Replace with your desired output (e.g. write to file, return as HTTP response, etc.)
    # return render_template("table_toggle_format.html", form=request_data, current_user=current_user)
